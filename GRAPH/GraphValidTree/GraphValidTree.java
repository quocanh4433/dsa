...
public class GraphValidTree {
     ...
     * 
     * Một graph (đồ thị) là một cây (tree) nếu nó thỏa 2 điều kiện:
     * 1. Connected (liên thông): Có đường đi giữa mọi cặp đỉnh.
     * 2. Acyclic (không có chu trình): Không có vòng lặp nào trong graph.
     * 
     * 
     * Time complexity: O(n)
     * 
    ...
    public boolean dfs(List<List<Integer>> graph, boolean[] visited, int node, int parent) {
        ...
        for(int nei : graph.get(node)) {
            ...
            if(!dfs(graph, visited, nei, node)) return false;
        }
        return true;
    }
